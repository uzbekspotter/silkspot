    ...
    <nav className="glass fixed top-0 w-full z-50" style={{ height: 52 }}>
      <div className="flex items-center h-full site-w gap-8" style={{ paddingLeft: '2rem', paddingRight: '2rem' }}>
        ...
        <button onClick={() => setCurrentPage('explore')}
          className="font-headline shrink-0 text-sm font-semibold tracking-tight transition-colors"
          ...
          {user ? (
            <div className="relative">
              <button onClick={() => setUserMenuOpen(v => !v)}
                ...
                <span className="hidden sm:block text-xs" style={{ color: '#ffffff' }}>{user.displayName}</span>
                <ChevronDown className="w-3 h-3 hidden sm:block" style={{ color: '#cbd5e1' }} />
              </button>
              <AnimatePresence>
                {userMenuOpen && (
                  <motion.div initial={{ opacity: 0, y: -4, scale: 0.97 }}
                    ...
                    style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 14, width: 196, boxShadow: '0 8px 30px rgba(0,0,0,0.12)' }}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={onSignIn}
                style={{ height: 32, padding: '0 14px', fontSize: 12, background: '#132337', color: '#ffffff', border: '1px solid #2d4a63', borderRadius: 6, cursor: 'pointer', fontWeight: 500 }}>Sign in</button>
              <button onClick={onSignUp}
                style={{ height: 32, padding: '0 14px', fontSize: 12, background: '#f8fafc', color: '#0f172a', border: '1px solid #e2e8f0', borderRadius: 6, cursor: 'pointer', fontWeight: 500 }}>Register</button>
            </div>
          )}
        </div>
      </div>
