...
- **2026-04-06** — **Upload (batch aircraft):** ускорение Submit — **кэш** `resolveOperatorId` / `resolveAircraftTypeId` по ключу; загрузка в R2 и `photos.insert` **параллельно** (`Promise.all`) после одного прохода по `aircraft` (порядок патчей борта без изменений). Раньше всё шло строго по одной фото. Файл: `UploadPage.tsx`. Коммит: `PENDING`.
