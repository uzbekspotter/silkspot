...
- **2026-04-06** — **Terms & Privacy:** страницы **`/terms`** и **`/privacy`** (markdown в `docs/terms_of_service.md`, `docs/privacy_policy.md`, UI `LegalDocPage.tsx`); маршруты в `app-path.ts` и `Page`; из **регистрации** ссылки открывают документ и **Back** возвращает в модалку или `/login`/`/register`; в футере — пункты Account. Коммит: `PLACEHOLDER`.
