...
- **2026-04-06** — **Сброс к стандартному типу на карточке борта:** в **Correct or add details** кнопка **Use standard catalog name** (подставляет имя из `aircraft_types`, после Save очищается `type_variant_label`) и короткая подсказка про Prestige/BBJ. Файл: `src/components/AircraftDetailPage.tsx`.
