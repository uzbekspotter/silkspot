...
- **2026-04-06** — **Лимит загрузок:** не более **50 новых строк `photos` на пользователя за календарный день UTC** — триггер `photos_daily_upload_limit` в миграции **`033_daily_photo_upload_limit.sql`**; на клиенте проверка до загрузки в R2 (`UploadPage`, `useUpload`), константа **`DAILY_PHOTO_UPLOAD_LIMIT`** в `src/lib/upload-limits.ts`. После деплоя кода **применить миграцию в Supabase** (CLI или SQL Editor). Коммит: `b82bb5c`.
