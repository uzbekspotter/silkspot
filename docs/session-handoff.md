...
- **2026-04-06** — **Профиль → Achievements:** блок **«Your badges»** — **ряд из N слотов** (по числу достижений): закрытые — **пунктир + бледная иконка**, открытые — цвет + **галочка**; подпись-мотивация и счётчик `k/n`. Карточки снизу с пометкой Locked/Unlocked. Файл: `ProfilePage.tsx`. Коммит: `PENDING`.
