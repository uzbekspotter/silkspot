...
- **2026-04-06** — **Save на карточке борта «не применялся»:** после успешного `UPDATE aircraft` всегда вызывается **`load()`**. Раньше перезагрузка данных была только если прошёл RPC **`set_operator_for_aircraft_photos`**; при ошибке RPC (например **Screener** без `is_moderator`, или сбой сети) интерфейс оставался старым — казалось, что тип не сохранился. Файл: `src/components/AircraftDetailPage.tsx`.
